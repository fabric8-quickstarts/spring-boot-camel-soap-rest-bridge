...
----
We will install this quickstart in openshift project|namespace, as well as the RH SSO image, so we need to add cluster-admin role to user developer.
...
----
Since Openshift 3.11, the new default docker image server registry.redhat.io requires authentication
